...
// checked once per consuming language. There is one such claim here. `FlowHost` is
// `@Composable`, which the Compose compiler refuses to call outside a composition — for
// Kotlin. From Java it is a static method taking a `Composer`, and passing null crashes at
// runtime rather than failing to compile. `@JvmSynthetic` is what removes it from Java's
// view, and ACC_SYNTHETIC on the class file is the only place that is observable.
//
...
class JavaVisibilityTest
{
    ...
    fun `every Flow transition stays callable from Java`()
    {
        ...
        assertEquals(
            ...
            sortedSetOf("close", "getStack", "isPresented", "open", "pop", "push", "replace"),
            callable
        );
    }
}
