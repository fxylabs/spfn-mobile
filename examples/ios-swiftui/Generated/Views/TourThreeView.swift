...
/// The `tourThree` screen, drawn out of SPFNUI's components.
@MainActor
public struct TourThreeView: View
{
    @State private var model: TourThreeModel

    public init(model: TourThreeModel)
    {
    ...
    public var body: some View
    {
        Screen(title: "Pushed, three of three", scroll: true)
        {
            VStack(alignment: .leading, spacing: SPFNTokens.space4)
            {
                ...
                TextButton(
                    title: "back",
                    identifier: "tourThree.back",
                    onTap: { model.back() }
                )
                PrimaryButton(
                    title: "done",
                    identifier: "tourThree.done",
                    onTap: { model.done() }
                )
            }
