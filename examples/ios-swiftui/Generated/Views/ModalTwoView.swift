...
/// The `modalTwo` screen, drawn out of SPFNUI's components.
@MainActor
public struct ModalTwoView: View
{
    @State private var model: ModalTwoModel

    public init(model: ModalTwoModel)
    {
    ...
    public var body: some View
    {
        Screen(title: "Over everything, two of two", scroll: true)
        {
            VStack(alignment: .leading, spacing: SPFNTokens.space4)
            {
                ...
                TextButton(
                    title: "back",
                    identifier: "modalTwo.back",
                    onTap: { model.back() }
                )
                PrimaryButton(
                    title: "done",
                    identifier: "modalTwo.done",
                    onTap: { model.done() }
                )
            }
