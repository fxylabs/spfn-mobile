...
public enum SheetFullRoute: FlowRoute
{
    case fullOne
}

/// How this flow is presented, and therefore what a back on its last route means.
public let sheetFullEntry: FlowEntry = .sheet(detent: .full)

/// A factory, so the flow opens on the screen the spec named as its start.
@MainActor
public func SheetFullFlow() -> Flow<SheetFullRoute>
{
    Flow(initial: [.fullOne])
}
...
@MainActor
public struct SheetFullFlowHost: View
{
    ...
    {
        FlowHost(flow: container.sheetFullFlow, entry: sheetFullEntry)
        { route in
            switch route
            {
            case .fullOne:
                FullOneView(model: container.fullOneModel())
            }
        }
    }
}
