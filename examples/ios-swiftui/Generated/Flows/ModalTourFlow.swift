...
public enum ModalTourRoute: FlowRoute
{
    case modalOne
    case modalTwo
}
...
public let modalTourEntry: FlowEntry = .modal

/// A factory, so the flow opens on the screen the spec named as its start.
@MainActor
public func ModalTourFlow() -> Flow<ModalTourRoute>
{
    Flow(initial: [.modalOne])
}
...
@MainActor
public struct ModalTourFlowHost: View
{
    ...
    {
        FlowHost(flow: container.modalTourFlow, entry: modalTourEntry)
        { route in
            switch route
            {
            case .modalOne:
                ModalOneView(model: container.modalOneModel())
            case .modalTwo:
                ModalTwoView(model: container.modalTwoModel())
            }
        }
    }
}
