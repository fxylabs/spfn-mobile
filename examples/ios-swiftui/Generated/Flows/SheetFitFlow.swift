...
@MainActor
public struct SheetFitFlowHost: View
{
    ...
    {
        FlowHost(flow: container.sheetFitFlow, entry: sheetFitEntry)
        { route in
            switch route
            {
            case .fitOne:
                FitOneView(model: container.fitOneModel())
            }
        }
    }
}
