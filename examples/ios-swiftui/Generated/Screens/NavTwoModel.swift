...
/// The `navTwo` screen's state and rules, with no toolkit in sight.
///
...
@MainActor
@Observable
public final class NavTwoModel
{
    /// What this screen's write is doing.
    public private(set) var state: Busy = .idle

    private let flow: Flow<SheetNavRoute>

    public init(
        flow: Flow<SheetNavRoute>
    )
    {
        self.flow = flow
    }
    ...
    public var stack: [SheetNavRoute] { flow.stack }
