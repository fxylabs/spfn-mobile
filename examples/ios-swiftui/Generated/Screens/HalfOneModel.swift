...
/// The `halfOne` screen's state and rules, with no toolkit in sight.
///
...
@MainActor
@Observable
public final class HalfOneModel
{
    /// What this screen's write is doing.
    public private(set) var state: Busy = .idle

    private let flow: Flow<SheetHalfRoute>

    public init(
        flow: Flow<SheetHalfRoute>
    )
    {
        self.flow = flow
    }
    ...
    public var stack: [SheetHalfRoute] { flow.stack }
