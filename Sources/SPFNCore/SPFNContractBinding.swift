...
public struct SPFNContractBinding: Equatable, Sendable
{
    ...
    /// `supportedRange` when the pin is its minor's first release, and narrower once a
    /// later patch is pinned: the declared range still starts at the minor's floor, and
    /// this SDK calls operations only the pinned patch serves. For a pre-release pin it is
    /// the pinned version alone: the declared range would promise every core below the next breaking
