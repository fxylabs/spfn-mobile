...
One row was briefly not unresolved: the device sign-in row below, whose gate is a fixed
table of fifteen cells and which was met in full on 2026-09-01 by a person holding two
phones. It stayed a narrow claim on purpose — a proven sign-in path is not a proven
platform, so it never moved the iOS or Android row. The 2026-09-02 re-pin to contract
`0.10.0` retired it: the gate judges a receipt against the pinned contract version, the
2026-09-01 receipts name `0.9.0`, and so the gate refuses them. It is red on purpose and
stays red until a person re-runs the fifteen cells.
...
| Contract | exact imported digest and supported SemVer range | a contract outside the range raises an explicit upgrade error; no unknown-profile fallback | enforced — `0.13.2` is pinned by real digest from the SPFN primitives export (D17 resolved) at primitives commit `dc90f70d`; the bundle declares `>=0.13.0 <0.14.0` and the SDK admits `>=0.13.2 <0.14.0`, because its lower bound is the pinned version, and the range check is driven by a shared cross-platform vector table |
...
| iOS | Swift tools, Xcode, minimum iOS/macOS, tested device matrix | Linux Swift core + macOS Apple API + real-device release evidence | UNRESOLVED — *builds* with swift-tools 6.1 and Xcode 26.2 against iOS 17 and macOS 14 (baseline revised 2026-09-02; the previous floor was iOS 16 / macOS 13, and no row here ever promised 16). On 2026-08-07 an iPhone 14 Pro (iPhone15,2) on iOS 26.5.2 reported `custody=secureEnclave`; the nine lifecycle cells did not run on that phone because Maestro has no physical-iOS driver, so the gate is still unmet. On 2026-09-02 the Linux half of this row's gate ran for the first time: Swift 6.2.1 on Ubuntu 24.04 built `SPFNCore`, `SPFNGenerated`, `SPFNAuth` and `SPFNClient` and ran 274 tests with 20 skipped and 0 failures — the same suites that run on macOS, less the 14 provider-adapter rows (those two modules declare no Linux half) and two rows a Linux platform cannot host. The Apple API half and the real-device half are both still unmet, so the status word does not move. On 2026-09-01 the same model (iPhone 14 Pro, iOS 26.5.2) proved all ten iOS device sign-in cells — five cases for Apple and five for Google — against a real SPFN server; that is the row below, and it is a sign-in path rather than a platform, so this row is unchanged |
...
| Device sign-in | which provider × platform × case cells a real phone has proven, and the receipt for each | **retired by the 0.10.0 re-pin on 2026-09-02** — this row was **proven for contract `0.9.0`**: on 2026-09-01 a person drove all fifteen cells by hand on real phones against a real SPFN server, {iOS × Apple, iOS × Google, Android × Google} × {first-enroll, re-login, user-cancel, network-failure, server-reject}. The 27 receipts that run produced, retries and mis-declared attempts included, stay committed under `tools/device-receipts/runs/2026-09-01/` as history; they are no longer evidence for anything this repository pins. Hardware, from the run log rather than from the receipts: an iPhone 14 Pro on iOS 26.5.2, and a Samsung Android phone whose model and OS version were not recorded and are therefore not claimed. Android × Apple is exempt because no Android Apple adapter module exists. The re-pin to contract `0.10.0` retires the claim: evidence about a `0.9.0` wire contract says nothing about the next one. `tools/device-receipts/receipt-gate.sh` reads the pinned version out of `Contracts/upstream-provenance.json`, so it now refuses every committed receipt and will keep refusing until a fresh fifteen-cell run for the current `0.13.2` pin against a server built from primitives commit `dc90f70d` exists. That run is owned by a person and cannot be produced by an agent or by CI — it needs two real phones and a real server — so this row stays retired until they do it |
