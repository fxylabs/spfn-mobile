...
| Contract | exact imported digest and supported SemVer range | a contract outside the range raises an explicit upgrade error; no unknown-profile fallback | enforced — `0.10.0` is pinned by real digest from the SPFN primitives export (D17 resolved) at primitives commit `77fe6246`, the admitted window is `>=0.10.0 <0.11.0`, and the range check is driven by a shared cross-platform vector table |
