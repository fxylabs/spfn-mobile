...
# became "only what tools/module-graph.json declares". On the Swift side that is section 7's
# package rule; on the Android side it is Gradle's dependency verification, which refuses
# any artifact gradle/verification-metadata.xml does not record. The module graph itself is
# a READ — a read that cannot run reports the same green as one that found nothing
# (docs/IMPLEMENTATION-PITFALLS.md P7). So the probe asks two separate questions: does each
# rule bite, and does the graph read fail red when it cannot run.
#
...
# the files. Each end of that guard gets a case, and so does the relaxation the same
...
#
# An Apple-only import in a module that builds on Linux, CryptoKit included, is no longer
# probed here: the Linux `swift build` in tools/ci/swift.sh refuses it, and a validator
# reading the same thing again was a second answer to a question the compiler owns.
#
# Two cases mutate a copy of the validator with its ROOT pinned, because their subject is
# what the check does when its own input is unavailable — the one condition that cannot be
...
expect_fail 'an iOS-only entry that claims an Android module is still checked' \
    'android/spfn-social-apple dependency edge matches the graph'
...
expect_unrunnable 'a skip widened to cover every module fails on the visit count' \
    'the android directory count matches module-graph.json' \
    's#^    if \[ -n "$android_module" \]#    if [ -n "" ]#'

# --- o. the decision this round settled are kept by a check, not by memory -----
