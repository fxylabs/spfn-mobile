...
class CaseTable(target: Target)
{
    ...
    private fun checklist(spec: Spec, cells: List<Cell>): String = buildString {
        ...
        appendLine();
        appendLine("${manual.size} cells with no runner, for one of two reasons. Most are a GESTURE or a");
        appendLine("resting height, which is the class of thing a device runner reports success for");
        appendLine("whether or not the platform read it as the gesture it meant — cells u7b and u10b");
        appendLine("spent a Mac round on exactly that (`docs/IMPLEMENTATION-PITFALLS.md` P22). The rest");
        appendLine("are here because a runner's TAP is not a finger: it is a down and an up with no");
        appendLine("movement between them, and a press that only a moving finger can cancel is one no");
        appendLine("runner can be pointed at (P36). So these are checked by a person on a real phone,");
        appendLine("and the answers are written down.");
        appendLine();
