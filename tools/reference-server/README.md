...
| `--test-clock` | run on a ticking, movable clock starting at this instant, which `/control/advance-clock` can move |
...
What the flag builds is `SpfnReferenceTickingClock`: **ticking, movable** — it starts at
the named instant, then runs at the rate of real time, and `advance` moves it on top of
that. The in-process unit suites use `SpfnReferenceTestClock` instead, which is frozen and
moves only by hand. Both implement `SpfnReferenceMovableClock`, which is what the control
route asks for. A launched server must tick because its client is in another process: the
SDK anchors its proof clock to one `core.time` answer and then derives time from its own
monotonic source, so against a server that stopped, every proof is future-dated within a
millisecond and refused as `PROOF_EXPIRED`.
...
| `POST /control/advance-clock` | moves an injected movable clock, ticking or frozen; refused with 409 on the system clock |
