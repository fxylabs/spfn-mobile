...
| D9 | First release version | **RESOLVED 2026-08-01** | `VERSION` is `0.1.0-alpha.2`, lockstep SwiftPM/Maven; 1.0.0 waits on Step 5 evidence (decision `01kyyw1yqy`). `0.1.0-alpha.1` was the first candidate and stays a public tag: never published, superseded because its commit predates the cold-cache verification-metadata fix, and tags are immutable here | nothing further |
