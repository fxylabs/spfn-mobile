...
| D2 | CI runner strategy | **PARTLY RESOLVED 2026-09-18** | every check that can run on Linux now runs under the CI referee: `swift`, `android` and `contract` are required checks on every pull request, each calling a script under `tools/ci/` so the runner and a developer's terminal execute the same text. What stays outside CI is macOS, real devices and publication — the Apple-framework half of the Swift suite, the emulator and device cells, and anything signed or uploaded | the open half is unchanged: macOS runner cost, a device farm and the attestation plan. A hosted macOS job and real-device evidence are still undecided, and `security.yml`, `release-candidate.yml` and `publish-central.yml` stay manual for that reason |
...
| D10 | Swift/Android module name asymmetry: `SPFNPersistence` vs `spfn-sync` | **RESOLVED 2026-08-04 by removal** | both modules were dropped from the train: never implemented, published as empty coordinates through `0.1.0-alpha.3`, and the asymmetric names were the clearest sign that neither side had decided whether the module was storage or synchronization | nothing further. A future local-persistence module is named when it is implemented, under the module rules in `docs/architecture/README.md` |
...
| D13 | Inter-module dependency edges | PROPOSED | generated and auth depend on core; client depends on core, auth and generated. The persistence and hybrid edges disappeared with their modules (D10) | whether these edges survive a future persistence module |
| D14 | CI action pinning | **RESOLVED 2026-09-18** | a workflow uses only the actions this unit needs, each pinned by commit SHA, with name and SHA recorded in `tools/ci/actions-allowlist.txt`. Today that is `actions/checkout` for the three gates and the pre-existing `actions/upload-artifact` for `publish-central.yml`. Nothing else: no `setup-java`, no `setup-gradle`, no Swift installer action and no `actions/cache` — toolchains arrive through `tools/ci/install-swift.sh` and `tools/ci/install-android-sdk.sh`, which pin what they fetch by digest, because a setup action pins nothing this repository can read. Section 24 of the validator reads every `uses:` line and fails on anything not on the list, and `tools/validate/probe-ci-actions-rules.sh` proves it refuses a tag ref, an unlisted SHA, a missing list and a tag inside the list itself | nothing further. Adding an action is an edit to the allowlist, which is a reviewed line, not a character change inside a YAML file |
...
| D17 | Upstream contract export tooling | **RESOLVED 2026-08-02** | SPFN primitives generates the bundle from its own route and contract definitions (`packages/auth/scripts/export-mobile-contract.ts`, tracked as primitives issue #48) and publishes it at `contracts/mobile/`. `Contracts/spfn-mobile-contract.json` is a byte copy pinned at commit `d31aa9a1`, contract `0.1.0`, digest `96c48f9c…`, with the exporter's own `Contracts/upstream-provenance.json` beside it. The lock is `RESOLVED_UPSTREAM` and the validator checks its claims against that evidence instead of refusing them. **Updated 2026-09-18:** `lockVersion` 3 shrank the lock to what only this repository can know — `source.commit`, `publishedPackages` and `contract.bundlePath` — and made `upstream-provenance.json` the sole source of the contract's own facts; the field-by-field lock-versus-evidence comparison, and the two gaps found in it on 2026-08-04, are gone with the second copy they compared, replaced by one closed rule that fails when a shed key reappears | nothing further. The Step 5 blocker is cleared: both platform suites and the integration matrix in external-target mode pass against a primitives dev server running this contract. Re-pinning a later export is the routine in `Contracts/README.md`, not a new decision |
...
| D20 | Repository ownership of the Android SDK location | **RESOLVED 2026-09-18** | the Android SDK is still found through `ANDROID_HOME` and no `local.properties` is committed. On a runner the components are named rather than assumed: `tools/ci/install-android-sdk.sh` installs `platforms;android-36` and `build-tools;36.0.0` with `sdkmanager` after accepting licences, uses the runner image's `sdkmanager` when there is one, and otherwise fetches the command-line tools zip pinned by SHA-256 in that script. A runner image that happens to carry the right platform today is not a pin | nothing further. Moving compileSdk or build-tools is an edit to that script beside the edit to the modules |

## Raised by the transport change set

| # | Decision | State | What exists | What a person must decide |
| --- | --- | --- | --- | --- |
| D21 | Cleartext (`http://`) policy for the transport | **RESOLVED 2026-09-18** | https, or http to loopback only — `localhost`, `::1`, `127.*`. The check is in one place, where a session is created, and therefore covers the unproven operations too: enrolment, login, a social token and a device-code start carry no proof and used to go out over cleartext to any host. `10.0.2.2` is refused with everything else, so the harness gives every Android target an `adb reverse` route to `127.0.0.1` | nothing further. The transport itself is still told where to send; the session is what refuses to be created |
| D22 | Response header fidelity on iOS | **RESOLVED 2026-09-18** | nothing above the transport depends on response header order or repetition, and none may start to. `HTTPURLResponse.allHeaderFields` is a dictionary, so both are already lost before the adapter sees them; the adapter sorts by lowercased name so the result is at least deterministic, and OkHttp preserves what iOS cannot | nothing further. A rule that ever needed order or repetition would need a different iOS response reader, not a different adapter — and would be a new decision, not this one |
...
| D24 | Client clock skew margin | **RESOLVED 2026-09-18** | no margin, because there is no skew to absorb: the proof clock is the server's own time, anchored through `core.time` and advanced by a sleep-inclusive monotonic source, so the device's wall clock never enters a timestamp or an expiry judgement. The residue that the anchor itself can go stale — a device that slept past the 300 000 ms replay window, or a server that moved its own clock — is closed by `discardAnchor` plus the `PROOF_EXPIRED` retry (w-fa0vf) | nothing further. A margin would now be a guess layered on a measurement |

## Raised by the module rules

| # | Decision | State | What exists | What a person must decide |
| --- | --- | --- | --- | --- |
| D25 | Local persistence and server sync in v1 | **RESOLVED 2026-08-04** | nothing, deliberately. The `SPFNPersistence` / `spfn-sync` modules were dropped rather than kept as empty coordinates (D10), and the pinned bundle declares no sync operation, so the module rule that a server round trip waits for the contract applies directly. Upstream `@spfn/storage` is server-side object storage and owns no device-local store, so there is no existing mechanism to adopt | nothing further for v1. A future local-persistence module is designed as an injected protocol first, is named when it is implemented, and needs an upstream contract operation before it can synchronize |
| D26 | Push in v1 | **RESOLVED 2026-08-04** | nothing, deliberately. No device-token registration, delivery or receipt surface exists on either platform. Upstream `@spfn/notification` implements email, SMS and Slack; `push` appears in its channel union type with no channel behind it, so the server half does not exist either, and the contract declares no push operation | nothing further for v1, and no activation condition is written down in advance — a condition stated before the requirement is known reads as a promise (the D11 principle) |

## Raised by the screen scaffold generator

| # | Decision | State | What exists | What a person must decide |
| --- | --- | --- | --- | --- |
| D27 | What a screen with a `source` does when it is shown AGAIN | OPEN | the two platforms do different things, and both are generated from rule R6, "a screen with a source loads it once when it appears, however it appeared" (`tools/ui-codegen/src/main/kotlin/xyz/superfunction/spfn/uicodegen/Rules.kt`). The Swift view writes `.task { await model.load() }`, which SwiftUI runs on every appearance — so a screen popped back to reads again. The Kotlin view writes `LaunchedEffect(model) { model.load() }`, which runs once per model instance — so a screen popped back to shows what it read the first time. The case table asserts the FIRST appearance, which both satisfy, so no cell moves whichever way this is answered | which behaviour R6 means. "Once when it appears" is the sentence both were written from and it does not distinguish an appearance from the first one. A re-read on every appearance is fresher and costs a request per navigation; a read per model is cheaper and can show a person a value the server has since changed. Answering it moves ONE of the two emitters and the rule's wording, and wants a cell that tells the two apart — which is why it is recorded rather than picked here |
