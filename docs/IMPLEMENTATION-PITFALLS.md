...
좁힌 뒤에는 "산문에만 있는 링크가 라우팅을 만족시키지 못한다"를 영구 probe로 남긴다.

**나온 곳.** cs-mzv14 r3. 리뷰어가 표 행에서 [P14](#p14) 링크를 지우고 같은 구역
산문에 넣자 검사가 14개 전부 라우팅됐다고 보고했다.
...
적는다. 원장이 장치를 실제보다 좋게 보이면 그 원장은 없느니만 못하다.

**나온 곳.** cs-mzv14 r3. "두 라운드가 전부 확인 안 한 저장소 사실이었다"고 적었으나
finding 2·4는 검사 구현 빈틈이었다. 리뷰어가 자기 이전 리뷰 기록으로 반증했다.
...
| cs-mzv14 (w-0r0ya) | 4 (cold 1) | 7 | 0 / 7 | P14 P15 P16 |
...
finding 7건은 전부 등록부가 아직 닿지 못하던 곳이었고, 그중 6건이 **등록부와 그
검사가 스스로 만든 결함**이다. 두 부류로 갈린다.

| 부류 | finding | 처방 |
| --- | --- | --- |
| 확인하지 않은 저장소 사실 | 1(digest 파일 수), 3(MANIFEST 갱신 방식), 5(upstream 증거를 파생물이라 부름) | 항목이 확인 명령을 함께 싣는다 |
| 검사가 자기 설명대로 동작하지 않음 | 2(앵커 맵 중복 붕괴), 4(앵커 없는 헤딩 미계수), 6(표 구역 산문이 행 행세) | 검사를 넣을 때 형제 케이스를 probe한다 |

한 부류로 적으면 다음 사람이 한 가지만 대비한다([P16](#p16)). 라운드가 8에서 4로
줄었지만 4가 아니라 1이 목표이고, 이 change set의 라운드는 전부 **등록부 자신을
