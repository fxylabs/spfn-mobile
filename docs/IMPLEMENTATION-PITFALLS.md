...
**저장소에 대한 사실을 주장하는 항목은 그 사실을 확인하는 명령을 함께 적는다.**
"digest는 세 곳에 있다", "이 파일은 손으로 갱신한다" 같은 문장은 쓰는 순간에는
맞아 보이고 시간이 지나면 조용히 틀린다. 브리프가 그것을 인용하므로 **틀린 항목은
없는 항목보다 나쁘다.**

**항목이 구현을 앞질러 쓰이지 않게 한다.** 자동 검사를 함께 넣는 항목은 그 검사가
실제로 하는 일만 적는다. 의도를 적고 구현이 못 따라가면, 그 항목도 틀린 항목이다.

**이 문서가 자기 이력을 서술할 때도 같은 규칙이 적용된다**([P16](#p16)). cs-mzv14의
finding 7건은 한 부류가 아니라 두 부류다 — 확인하지 않은 저장소 사실(1·3·5)과, 검사가
자기 설명대로 동작하지 않은 구현 빈틈(2·4·6). 둘은 처방이 다르므로 하나로 합쳐 적으면
다음 사람이 잘못된 대비를 한다.
...
| 이 문서 자체 수정, 등록부 라우팅 검사 수정 | [P14](#p14) [P15](#p15) [P16](#p16) [P7](#p7) |
...
**처방.** 세 수를 따로 센다. 헤딩 출현 횟수, 그중 앵커를 가진 것, 유일 앵커 수.

| 비교 | 다르면 |
| --- | --- |
| 헤딩 ≠ 앵커 있는 헤딩 | 앵커 없는 항목이 있다 — 읽는 사람에게는 항목인데 검사에는 보이지 않는다 |
| 앵커 있는 헤딩 ≠ 유일 앵커 | 앵커가 두 번 쓰였다 — 둘 중 하나에 도달할 수 없다 |

중복이 정당한 쪽(트리거 참조)에는 같은 규칙을 적용하지 않는다. 정당한 중복을 막는
검사는 보호하려는 문서를 왜곡한다.

**나온 곳.** cs-mzv14 r1이 앵커 중복을, r2가 앵커 없는 헤딩을 잡았다. r1의 수정이
헤딩 계수를 앵커 조건 **안에** 두는 바람에 앵커 없는 헤딩이 아예 세어지지 않았고,
이 항목의 초판이 "헤딩 출현 횟수를 따로 센다"고 적어 구현보다 앞서 있었다 — 항목이
구현을 앞질러 쓰이면 그것도 틀린 항목이다.

## P15. 표 구역 안의 산문이 표 행 행세를 한다 {#p15}

**증상.** 라우팅 검사가 트리거 표 **구역**(헤딩부터 다음 구분선까지)의 모든 줄에서
링크를 긁었다. 그래서 표 행에서 항목 링크를 지워도, 같은 구역 안 산문에 그 앵커가
한 번 나오면 "라우팅됨"으로 통과한다. 표에는 없는데 검사는 있다고 말한다.

**탐지.** 구조를 검사한다면서 **구역**으로 범위를 잡았는지, **구조 요소**로 잡았는지
본다. Markdown 표라면 행은 `|`로 시작한다. 헤딩부터 구분선까지를 범위로 삼는 순간
그 사이의 설명 문단·주석·예시가 전부 검사 대상이 된다.

**처방.** 추출 범위를 구조 요소로 좁힌다 — 표 행이면 `^[[:space:]]*\|`인 줄만.
코드 펜스 안은 예시이므로 아예 건너뛴다. 좁힌 뒤에는 그것을 영구 probe로 남긴다 —
`tools/validate/probe-register-routing.sh`가 산문 링크와 펜스 안 파이프 행을 각각
케이스로 들고 있다.

**나온 곳.** cs-mzv14 r3. 리뷰어가 표 행에서 [P14](#p14) 링크를 지우고 같은 구역
산문에 넣자 검사가 14개 전부 라우팅됐다고 보고했다. r4는 같은 구멍의 형제 케이스를
찾았다 — 펜스 안에서 `|`로 시작하는 예시 행도 표 행으로 읽혔다.

## P16. 회고 서술이 finding을 한 부류로 뭉갠다 {#p16}

**증상.** 원장이나 항목의 "나온 곳"이 여러 라운드의 finding을 하나의 근본 원인으로
정리한다. 정리는 읽기 좋지만, 실제로는 처방이 다른 두세 부류가 섞여 있다. 다음 사람이
한 가지만 대비하고 나머지를 그대로 밟는다.

**탐지.** 요약을 쓰기 전에 **finding을 하나씩 리뷰 아티팩트에 대조**해 부류를 붙인다.
붙인 부류가 하나뿐이면 그것이 진짜인지 의심한다 — 여러 라운드가 정말로 한 원인이면
두 번째 라운드에서 이미 끝났어야 한다.

**처방.** 부류가 둘 이상이면 둘 이상으로 적고, 각 부류의 처방이 어떻게 다른지 한 줄씩
적는다. **분류표가 자기 자신을 빠뜨리지 않았는지 센다** — finding 번호를 전부 나열해
표에 실린 개수와 맞춰 본다. 원장이 장치를 실제보다 좋게 보이면 그 원장은 없느니만 못하다.

**나온 곳.** cs-mzv14 r3. "두 라운드가 전부 확인 안 한 저장소 사실이었다"고 적었으나
finding 2·4는 검사 구현 빈틈이었다. 리뷰어가 자기 이전 리뷰 기록으로 반증했다. r4는
그 수정이 다시 불완전하다고 잡았다 — 두 부류로 나눈 표가 정작 자신을 만든 finding 7을
분류에서 빠뜨리고 있었다.
...
| cs-mzv14 (w-0r0ya) | 5 (cold 1) | 10 | 0 / 10 | P14 P15 P16 |

**cs-mzv14 읽는 법.** 등록부를 도입한 change set 자신이다. 브리프에 인용된 항목은
[P4](#p4)–[P7](#p7)이었고 리뷰가 세 라운드에 걸쳐 그 넷이 전부 지켜졌다고 확인했다 —
**인용된 항목을 놓쳐서 나온 finding은 0건**이다.

finding 10건은 **전부** 등록부와 그 검사가 스스로 만든 결함이다. 놀랄 일은 아니다 —
이 change set의 내용이 곧 등록부이므로 다른 표면이 없다. 두 부류로 갈린다.

| 부류 | finding | 처방 |
| --- | --- | --- |
| 확인하지 않고 단언한 사실 | 1(digest 파일 수), 3(MANIFEST 갱신 방식), 5(upstream 증거를 파생물이라 부름), 7(회고가 부류를 하나로 뭉갬), 8(그 회고가 자기 자신을 분류에서 빠뜨림) | 주장마다 확인 명령·근거를 함께 싣는다. 저장소 사실이든 자기 리뷰 이력이든 같다 |
| 검사·항목이 자기 설명대로 동작하지 않음 | 2(앵커 맵 중복 붕괴), 4(앵커 없는 헤딩 미계수), 6(표 구역 산문이 행 행세), 9(펜스 안 파이프 행이 행 행세), 10(P15가 처방한 영구 probe가 없었음) | 검사를 넣을 때 형제 케이스를 probe하고, 처방한 것을 실제로 남긴다 |

한 부류로 적으면 다음 사람이 한 가지만 대비한다([P16](#p16)). 라운드가 8에서 5로
줄었지만 5가 아니라 1이 목표이고, 이 change set의 라운드는 전부 **등록부 자신을
정확하게 만드는 데** 쓰였다 — 등록부가 존재한 뒤 첫 실제 구현 change set이 몇
라운드에 끝나는지가 진짜 측정값이다. 그 숫자가 나오기 전까지 이 원장은 수렴을
보여주지 않는다.
