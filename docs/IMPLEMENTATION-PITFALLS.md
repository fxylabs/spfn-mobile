...
요청인가, 플로우가 떠 있는가, **이 화면 자신의 라우트가 아직 스택에 있는가.** 셋째가 없으면
적중이다.

```
# 생성된 모델의 가드가 라우트까지 보는가. 화면 수만큼 나와야 한다.
...
**처방.** 가드에 라우트 검사를 더한다: 페이로드가 있는 라우트든 없는 라우트든
`stack.contains(자기 라우트 값)` 하나로 같은 질문이 된다. 셋 다 필요하고 어느 하나도 나머지를
함의하지 않으므로, 표에는 **셋을 갈라 세우는 셀**을 둔다 — 라우트가 빠진 뒤의 응답(u8d),
플로우가 닫혔다 다시 열린 뒤의 응답(u1c), 그리고 플로우가 닫힌 채인 응답(u8c/u9c). 가드에서
하나를 지웠을 때 정확히 하나가 붉어지면 셋이 서로 다른 것을 지키고 있다는 뜻이다.
