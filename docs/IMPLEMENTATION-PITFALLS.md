...
| push 플로우의 호스팅·`NavigationHost`·호스트 스택 수정, 플로우가 자기 내비게이터를 만드는 코드 | [P31](#p31) [P30](#p30) [P15](#p15) [P25](#p25) |
...
**iOS 18 이하 런타임에서는 가장자리 스와이프도 조용히 성공한다.** iOS 18.2 시뮬레이터에서
`swipe: start "1%, 50%"`(그리고 `0%`, 좌표 `2, 500`)는 `COMPLETED`로 끝나지만 화면은 한
단계도 돌아가지 않는다. SDK 없이 시스템 바만 쓰는 순수 SwiftUI 앱에서도 똑같이 돌아가지
않았고, 같은 화면을 사람이 마우스로 끌면 돌아갔다. 그러니 앱이 아니라 러너의 한계다. iOS
26.3에서는 같은 셀(C1·C3·C10)이 통과한다. 17·18 런타임에서 이 셀들의 실패는 결함이 아니고,
그 런타임의 가장자리 스와이프는 사람이 확인한다. C2(내용 영역 스와이프)는 이와 별개로 iOS
26에만 있는 제스처라 17·18에서는 실패하는 것이 맞다.
...
## P31. 호스트 위에 자기 스택을 그려 덮는 push는 전환도 없고 루트에서 돌아갈 길도 없다 {#p31}

**증상.** push로 여는 플로우를 메뉴에서 열면 첫 화면이 **전환 애니메이션 없이** 그 자리에
나타난다. 그리고 그 첫 화면에는 메뉴로 돌아갈 길이 없다 — 헤더에 뒤로가기가 그려지지 않고,
가장자리 스와이프도 시스템 뒤로가기도 아무 일을 하지 않거나 앱을 닫는다. 규칙 표에
"push 루트의 뒤로는 호스트의 뒤로"라고 적혀 있어도 마찬가지다: 받을 호스트가 없다.

원인은 표시 방식이 아니라 **스택이 두 개**라는 것이다. `FlowHost(.push)`가 자기
`NavigationStack`/`NavDisplay`를 만들어 호스트 위에 `ZStack`/`Box`로 얹으면, 그 플로우의
루트는 새 내비게이터의 루트다. 루트 아래에는 아무 라우트도 없으니 (1) 밀려 들어올 화면이
없어 전환이 없고, (2) 팝할 대상이 없어 뒤로가기가 그려질 이유도 없다. "호스트의 뒤로가
적용된다"는 문장은 호스트의 내비게이션과 플로우의 내비게이션이 **같은 스택일 때만** 참인데,
이 구조에서는 두 스택이 겹쳐 있을 뿐 이어져 있지 않다.

**탐지.** push 플로우를 여는 코드 경로에서 내비게이터를 **누가 만드는지** 센다. 호스트 앱의
루트에 하나, `FlowHost`에 또 하나면 이미 두 개다. 기기 없이 보는 법: 플로우의 루트 화면에서
헤더 뒤로가기 컨트롤이 그려지는지, 그리고 그 컨트롤이 없다면 그 화면을 떠나는 다른 컨트롤이
스펙에 있는지 확인한다 — 둘 다 없으면 사람이 갇힌다. 셀로 고정하는 법: 루트에서 헤더 뒤로와
시스템 뒤로를 각각 눌러 `stack=0`과 **호스트의 컨트롤**(`menu.<flow>`)을 함께 기대하는 셀을
쓴다. `stack=0`만 기대하면 빈 화면으로 닫힌 플로우도 통과한다.

**처방.** push는 호스트의 스택에 **이어 붙인다**. SDK가 호스트용 컨테이너를 양 플랫폼 같은
이름으로 준다(`NavigationHost`), 그 안의 `FlowHost(.push)`는 자기 내비게이터를 만들지 않고
자기 라우트를 호스트의 목록(`HostStack`)에 올린다. 그러면 전환은 플랫폼 자신의 push 전환이
되고, 루트 아래에는 호스트의 화면이 있으므로 루트의 뒤로가 의미를 갖는다 — 그 뒤로는
**플로우의 close**다(결정 N2). 호스트 없이 쓰는 경로는 호환을 위해 남기되, 전환도 호스트
복귀도 없다는 것을 그 파일의 헤더에 적는다.

두 가지가 함께 따라온다. 첫째, 호스트의 스택에는 **여러 플로우의 라우트가 섞일 수 있으므로**
항목마다 주인을 적고, 플랫폼이 꼬리를 자른 뒤 알려주는 "길이"를 주인별 개수로 되돌리는
산술을 툴킷 없는 파일에 두고 양쪽에서 테스트한다(`HostStack.shortened`). 둘째, 안드로이드의
`NavDisplay`는 **보이는 씬만 컴포즈**하므로 호스트 루트 위로 라우트가 하나 얹히는 순간 호스트
루트의 컴포지션(메뉴와 그 안의 모든 `FlowHost`)이 폐기된다. 등록을 `DisposableEffect`의
`onDispose`에서 해제하면 첫 라우트가 루트를 덮는 순간 그 플로우의 항목이 스택에서 빠지고,
호스트는 루트로 되돌아가고, 둘이 계속 싸운다 — 등록은 한 번 하고 스택 추적은 호스트 자신의
스코프에서 도는 코루틴이 맡아야 한다.

**나온 곳.** w-evwna 3e, iPhone 14 Pro, 2026-09-04. 사람이 `pushTour`를 열고 첫 화면에서
빠져나오지 못했다. 3d까지의 기기 게이트는 전부 초록이었는데, 표의 push 루트 행이 "none"과
"the host app's"였기 때문에 **아무도 그 자리를 물어보지 않았다** — 셀이 없는 칸은 통과하는
칸이 아니라 보지 않는 칸이다.

## P32. 팝이 드러낸 아래 화면은 팝이 끝나기 전에 깨어난다 — 거기서 제스처를 끄면 그 팝이 취소된다 {#p32}

**증상.** 가장자리 스와이프 뒤로가기가 **스택 깊이 2 이상에서는 되는데 플로우의 첫 화면에서만**
안 된다. 스와이프 자체는 받아들여진다: 아래 화면이 실제로 드러나고, 접근성 트리에도 아래
화면의 컨트롤이 보인다. 그런데 손을 떼면 화면이 도로 덮이고 플로우는 열린 채로 남는다.
러너에서는 스와이프 명령이 아니라 **그 다음 단언**이 깨진다.

원인은 제스처 인식기의 소유자가 **화면이 아니라 내비게이션 컨트롤러**라는 데 있다. 화면마다
"나는 뒤로가 있다/없다"를 보고 인식기를 켜고 끄면, 켜는 쪽과 끄는 쪽이 같은 인식기 하나를
두고 다툰다. 그리고 팝은 아래 화면을 **커밋 전에** 윈도에 되돌려 놓는다 — 사람이 밀려서
보고 있는 것이 그 화면이기 때문이다. 뒤로가 없는 화면(호스트 앱의 메뉴 같은)이 그 순간
`didMoveToWindow`로 깨어나 인식기를 끄면, 자기를 드러내던 팝을 자기가 취소한다.

깊이 2 이상에서 안 보이는 이유도 같은 문장이다. 그 아래에 있는 것은 같은 플로우의 다른
화면이고, 그 화면에는 뒤로가 있으므로 인식기를 도로 **켜** 준다.

**탐지.** 제스처 인식기·리스너·핸들러를 화면 단위 수명주기(`didMoveToWindow`,
`onAppear`/`DisposableEffect`)에서 **끄는** 코드를 찾는다. 끄는 주체가 여럿이면 이미 위험하다.
...
`delegate`를 `nil`로 비우는 흔한 요령은 여기서 두 번 틀린다. 스택이 하나짜리일 때도 제스처가
시작되고, 인식기를 켠 화면이 사라진 뒤에도 켜진 채로 남는다. 또 `delegate`는 weak이므로
화면마다 델리게이트를 만들면 그 화면과 함께 사라진다 — 공유 객체 하나여야 한다.

**나온 곳.** w-evwna 3e, iOS 시뮬레이터, 2026-09-04. [P31](#p31)의 처방으로 push 플로우가
호스트 스택에 올라가자, 그 플로우 루트 아래에 처음으로 **뒤로가 없는 화면**(호스트의 메뉴)이
놓였다. `pushTour-rootSystemBack` 셀 하나만 실패하고 `u7b`·`u10b`는 초록이었다 — 하나를
고치면 다음 셀 실행이 그 아래 있던 것을 새로 비춘다는 [P29](#p29)·[P30](#p30)의 순서가
한 번 더 반복됐다.

## P33. 호스트 컨테이너의 root는 그 뒤에 밀려 들어오는 화면의 부모가 아니다 {#p33}

**증상.** push로 여는 플로우의 화면에서 **id 셀렉터만** 안 잡힌다. 같은 화면의 텍스트는
정상으로 잡히고, 모달·시트 플로우는 전부 초록이다. 러너 로그는 `Element not found: Id ...`
하나뿐이라 화면이 안 그려진 것처럼 읽히지만, 화면은 그려져 있다.

원인은 **형제인데 자식으로 착각한 것**이다. `NavigationHost`(안드로이드에서는 `NavDisplay`,
iOS에서는 `NavigationStack`)에 넘긴 `root`는 백 스택의 **항목 하나**이고, push된 플로우의
라우트들은 **다른 항목들**이다. 둘은 나란히 서 있지 형제-부모가 아니다. 그래서 앱이 자기
루트에 감아 둔 `Modifier`·`CompositionLocalProvider`·`semantics`는 push된 화면 위에 있지
않다.

Compose에서 이게 조용히 비싸다. `testTagsAsResourceId`는 Maestro의 `id:`가 매칭하는 안드로이드
리소스 id로 test tag를 바꿔 주는 스위치인데, **semantics 부모를 거슬러 올라가며** 해석된다.
호스트 안쪽에 걸면 앱 자기 화면에서는 살아 있고 push된 플로우의 모든 컨트롤에서 사라진다 —
텍스트 단언은 그대로 통과하므로 셀 실패가 "화면이 없다"로 보인다.

**탐지.** 호스트 컨테이너에 `root`를 넘기는 자리에서, 그 `root`에 붙은 것 중 **네비게이션
전체에 걸려야 하는 것**이 있는지 센다. 셀로 갈리는 모양: 같은 화면에서 텍스트 단언은 통과하고
id 단언만 실패하면 상속되는 스위치를 먼저 의심한다. push 플로우가 하나뿐인 표는 이걸 못 본다 —
push 플로우 **전부**가 같은 자리에서 같이 죽는 것이 신호다.

**처방.** 앱 전체에 걸려야 하는 것은 호스트 **바깥**에 감는다. 안쪽에 남는 것은 앱 자기
화면의 것뿐이다 — 인셋이 그렇다. push된 `Screen`은 자기 위에서 아무도 소비하지 않은 상태 표시줄
인셋을 자기 헤더로 직접 쓰기 때문이다([P25](#p25)). 호스트 컨테이너를 제공하는 SDK는 이
문장을 자기 파일 머리말에 적는다: **root에 감은 것은 root에만 걸린다.**

**나온 곳.** w-evwna 3e, Android 에뮬레이터, 2026-09-04. [P31](#p31)의 처방으로 예제 앱이
`NavigationHost`를 도입하면서, 원래 앱 루트 Box에 있던 `testTagsAsResourceId`가 호스트 안쪽에
갇혔다. push 플로우 3개(`pushTour`·`keyboardForm`·`longScroll`)의 셀 5개가 한꺼번에 죽었고
나머지 30개는 초록이었다. iOS는 접근성 식별자가 뷰마다 따로 붙으므로 같은 커밋에서 아무 일도
일어나지 않았다 — **한쪽 플랫폼만 상속으로 해석하는 속성**은 두 반쪽이 같은 코드를 갖고도
다르게 부서지는 자리다.

## P34. 유한한 최대 제약 안의 `fillMaxSize`·`weight`는 wrap을 상한으로 만든다 {#p34}

**증상.** "내용 높이만큼" 서기로 한 컨테이너가 **상한 높이로** 선다. 갤럭시 Z 플립4(SM-F721N,
Android 15)에서 Fit 시트의 상단이 y≈290/2640 — Full 시트와 같은 자리 — 에 섰고, 그 안의 내용은
y≈1130에서 끝났다. 아래 900px 넘는 흰 여백이 시트의 일부로 그려진다. 크래시도, 경고도, 실패하는
단언도 없다. 시트는 열리고 닫히고 컨트롤은 전부 눌린다.

원인은 **wrap이 무엇을 재는가**이다. `Modifier.wrapContentHeight().heightIn(max = full)`은 "내용을
재고 상한에서 자른다"로 읽히지만, 재는 대상이 **자기가 받은 만큼을 채우도록 쓰인 자식**이면 그
자식은 상한까지 자란다. wrap은 상한을 잰다. Compose의 `Screen`은 루트가 `fillMaxSize()`였고 본문이
`weight(1f)`였으므로 정확히 그랬다.

`heightIn(max)`의 최대가 **유한**하다는 것이 조건의 전부다. 같은 코드가 무한 제약(스크롤 컨테이너
안, `Constraints.Infinity`) 아래에서는 옳게 동작한다 — `fillMaxSize`는 무한을 채울 수 없으니
자식의 자연 높이로 떨어진다. 그래서 이 결함은 상한이 도입되는 순간 나타나고, 상한 없이 쓰던 자리에서
옮겨 온 코드에서 특히 잘 나타난다.

**탐지.** 화면이 아니라 **높이 두 개를 비교해서** 잡는다. 같은 내용을 Fit과 Full로 각각 띄우고
상단 y가 같으면 이 항목이다. 스크린샷 한 장만으로는 "내용이 원래 그만큼인가" 와 구분되지 않으므로
내용의 끝 y도 같이 읽는다. 코드에서의 신호: 유한한 `heightIn(max=…)`·`sizeIn`·`BoxWithConstraints`
안쪽 어딘가에 `fillMaxSize`·`fillMaxHeight`·`weight(1f)`가 있는가. 자동 검사로는 잡히지 않는다 —
이 저장소에는 Compose UI 테스트 인프라가 없고(로보렉트릭도 계측 스위트도 없다), 레이아웃은 컴파일도
JVM 단위 테스트도 통과한다.

**처방.** 상한을 두는 쪽이 **그 안에 있다는 사실을 아래로 알린다**. `Sheet`가
`LocalFitsContent`(internal 컴포지션 로컬)로 Fit인지 말하고, `Screen`이 그것을 읽어 루트를
`fillMaxHeight()`가 아닌 `wrapContentHeight()`로, 본문을 `weight(1f)`가 아닌
`wrapContentHeight()`로 둔다. **두 항 다** 바꾼다 — 루트만 wrap하고 본문이 계속 채우면 wrap이 재는
자식이 여전히 상한을 먹은 자식이라 결과가 같다.

기본값은 `false`, 즉 "채운다"여야 한다. Fit 시트 밖의 모든 화면 — push, modal, half, full, 플로우
없이 호스트 앱이 직접 그린 `Screen` — 이 그 기본값을 읽고, 짧은 화면의 배경이 창 끝까지 칠해지는
것이 거기에 달려 있다.

분기는 컴포저블 안의 `if`가 아니라 **값**으로 둔다. 이 저장소에서 컴포저블 안에 쓴 분기는 JVM에서
읽을 수 없으므로, `ScreenLayout.forDetent(fits: Boolean)` 같은 순수 함수로 빼면 4셀짜리 단위
테스트가 생긴다. 그 테스트가 지키는 것은 "modifier가 그렇게 레이아웃된다"가 아니라 — 그건 기기가
지킨다 — 리팩터가 조용히 뒤집는 쪽, 즉 `Fill`이 Fit 분기로 새거나 `Wrap`이 밖으로 새는 것이다.

크기 제약과 스크롤의 **순서**도 여기에 딸린다. `wrapContentHeight` → `verticalScroll` 순서라야
스크롤 노드가 무한 높이로 자식을 재고 자기가 받은 높이와의 작은 쪽을 취한다. 반대로 감으면 상한이
아무것에도 걸리지 않는다.

**나온 곳.** w-3c8nk 3g, Galaxy Z Flip4 (SM-F721N) / Android 15, 2026-09-07. `sheetFit-detent`
셀은 초록이었다 — 그 셀이 읽는 것은 readout과 컨트롤이고, 시트가 얼마나 높은지는 아무 단언도 읽지
않는다. 사람이 스크린샷을 보고 잡았다.

## P35. 예측 뒤로가기는 Android 13~15에서 매니페스트 선언 없이는 진행률을 주지 않는다 {#p35}

**증상.** 뒤로가기 제스처를 **홀드하는 동안** 아무 일도 일어나지 않는다. 시스템 화살표만 뜨고 현재
화면은 그대로 있으며, 손을 놓는 순간 아래 화면이 나타난다. 가장자리에서 놓으면 정상적으로 취소된다.
즉 **뒤로가기는 완전히 정상이고 애니메이션만 없다.**

원인은 코드가 아니라 **선언**이다. `NavDisplay`의 `predictivePopTransitionSpec`(SwiftUI라면
`NavigationStack`의 대응물)은 제스처의 **진행률**을 받아야 도는데, 진행률은 창의
`OnBackInvokedDispatcher`를 통해서만 온다. 시스템이 제스처를 거기로 보내는 조건이
`<application android:enableOnBackInvokedCallback="true">`이고, **Android 13·14·15에서는 앱이
targetSdk 36이어도 이 값이 기본 꺼짐**이다. 기본값이 켜지는 것은 Android 16부터다.

선언이 없으면 제스처는 레거시 KEYCODE_BACK 경로로 가고, androidx의 `OnBackPressedDispatcher`는
**완료된** back만 받는다. 그래서 팝은 일어나고 스택 깊이는 맞고 스택 깊이를 읽는 셀은 전부 초록이다.
없어진 것은 애니메이션뿐이고, 이 저장소의 어떤 단언도 애니메이션을 읽지 않는다.
...
`BackHandler`의 의미는 두 경로에서 같다는 것을 확인하고 쓴다. `androidx.activity.compose.BackHandler`
(1.12.4)는 `OnBackPressedCallback`을 등록하고, `ComponentActivity`가 API 33 이상에서
`OnBackPressedDispatcher.setOnBackInvokedDispatcher`로 창의 디스패처를 물려 준다. 활성화된 콜백은
어느 경로에서든 똑같이 제스처를 가져간다. 콜백 경로가 더 주는 것은 `onBackStarted`·`onBackProgressed`
뿐이고 `BackHandler`는 그 둘을 구현하지 않는다 — 진행 중에는 아무것도 하지 않고 놓을 때 동작한다.
홀드 중에 그리고 싶으면 `PredictiveBackHandler`가 그 자리다.

새 검사를 넣을 때는 두 앱을 **명시 목록**으로 적고 읽은 개수를 그 목록에 대조한다([P7](#p7)).
그리고 매니페스트에서 주석을 먼저 걷어낸 뒤 속성을 찾는다 — 이 저장소의 두 매니페스트는 바로 이
플래그를 긴 산문으로 설명하고 있어서, 단어만 찾는 검사는 아무것도 선언하지 않은 파일에서 그 단어를
찾아낸다.

**나온 곳.** w-3c8nk 3g, Galaxy Z Flip4 (SM-F721N) / Android 15, 2026-09-07. `pushTour-predictiveBack`
...
## P42. 2xx인데 읽을 수 없는 응답은 실패가 아니라 결과 불명이다 {#p42}

**증상.** 서버 상태를 바꾸는 호출 하나가 나간 뒤 설치가 **멈춘다.** 키 회전이라면: 회전
직후부터 모든 증명이 거부되고, 재등록 말고는 길이 없다. 앱 로그에는 디코딩 실패 하나뿐이고
서버 로그에는 **회전이 성공으로 적혀 있다.** 두 로그가 서로 다른 이야기를 한다.

**왜.** 오류 처리가 2xx 경로와 비2xx 경로를 한 catch로 받았기 때문이다. 두 경로는 정반대를
뜻한다.

| 온 것 | 서버가 한 일 | 클라이언트가 해야 할 일 |
| --- | --- | --- |
| 비2xx인데 본문을 못 읽음 | **아무것도 안 했다** — 거부는 거부다 | 후보·임시 상태를 지운다 |
| 2xx인데 본문을 못 읽음 | **했을 수도 있다** — 서버는 "됐다"고 답했다 | 후보를 **남기고** 재확인 경로로 넘긴다 |

두 번째 줄을 첫 번째처럼 다루면, 서버가 방금 등록한 키를 클라이언트가 지운다. 옛 키는 이미
서버에서 죽었고 새 키는 로컬에서 사라졌으니 남는 것은 wipe + 재등록뿐이다.

**왜 실패 케이스가 아니라 결과 불명인가.** 본문을 못 읽는 이유는 프록시가 끼워 넣은 응답,
계약 스큐, 서버의 부분 배포 등 **응답 경로의 문제**다. 요청 경로에서 아무 일도 없었다는
증거가 되지 못한다. 전송 실패와 같은 값이다 — 답을 못 받았다.

**탐지.** 세 가지가 모두 해당하면 이 함정이다.

1. 그 호출이 **서버 상태를 바꾸는가.** 조회는 해당 없다.
2. 디코딩 실패가 2xx와 비2xx **양쪽에서** 나올 수 있는가. 실패 이름만으로는 구간을 알 수
   없는 경우가 많다 — 이 SDK의 `notCanonicalJSON`이 그렇다.

   ```sh
   grep -n 'notCanonicalJSON\|NOT_CANONICAL_JSON' Sources/SPFNClient/SPFNClient.swift \
       android/spfn-client/src/main/kotlin/xyz/superfunction/spfn/client/SpfnClient.kt
   ```

   두 경로에서 같은 값이 나오면 오류 타입에 **상태 구간을 실어야** 한다.
3. 정리 코드가 `default`·`else` 분기에 있는가. 있으면 그 분기가 2xx도 받는다.

**처방.** 오류 타입이 상태 구간을 들게 하고 — 이 SDK는
`SPFNClientError.decoding(SPFNDecodingFailure, onSuccessStatus: Bool)` /
`SpfnClientError.Decoding(failure, onSuccessStatus)` — 분류를 **함수 하나로** 뺀다. 그
함수가 진입점 둘(회전과 재개)에서 읽히면 표가 두 벌로 갈라지지 않는다. 회전 표는
docs/architecture/README.md의 K1~K10이고, 이 함정이 만든 행은 **K3(2xx 디코딩 → 후보
유지)**와 **K4(비2xx 디코딩 → 후보 삭제)**다.

분류 함수는 **전체 함수(total)**여야 한다. 클라이언트 오류 타입도 인증 오류 타입도 아닌
것 — 동기화되지 않는 시계, 쓰이지 않는 키 저장소 — 이 catch를 빠져나가면 후보가 남고
상태가 "보류"라고 답한다. **보내지도 않은 요청에 대해서.** 그쪽은 전송 전 실패이므로 후보를
지우는 행(K8)이다.

그리고 **2xx를 읽은 뒤의 코드는 분류 대상이 아니다.** 승격·저장·id 비교는 서버가 이미
적용한 뒤의 장부 정리이므로, 거기서 나는 모든 실패는 후보를 그대로 둔다(K9). 코드로는
`send`만 catch로 감싸고 승격은 밖에 두는 모양이다.

**나온 곳.** w-ethjf, 2026-09-18. `rotate()`가 `SPFNClientError`·`SPFNAuthError`만 잡고
...
처방으로 예제 앱이 호스트를 도입하면서 드러난 것이 에뮬레이터 라운드의 [P33](#p33)이다.
셋은 한 줄로 이어진다 — **push를 호스트 스택에 올린 것 하나가, 그 아래 처음으로 뒤로 없는
화면을 놓았고(P32) 그 옆에 처음으로 상속되지 않는 형제를 놓았다(P33)**. 3d의 읽는 법과 같은
문장이 세 번째로 반복된다: 하나를 고치면 다음 셀 실행이 그 아래 있던 것을 새로 비춘다.

한 가지가 3d와 다르다. P33은 두 플랫폼이 **같은 커밋에서 다르게 부서졌다** — iOS는 아무 일도
없었고 Android만 셀 5개가 죽었다. 어휘가 같고 코드가 같아도 플랫폼 하나만 상속으로 해석하는
속성이 있으면 대칭은 거기서 끝난다. section 13의 이름 대조로는 잡히지 않는 자리이므로,
validate.sh section 14에 줄 번호 비교 검사를 넣었다.
