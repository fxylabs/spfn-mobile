...
없는 항목보다 나쁘다.** cs-mzv14의 리뷰 두 라운드가 전부 이 부류였다 — 한 번은
파일 개수가, 한 번은 갱신 방식이 확인 없이 적혀 있었다.
...
| 역할 | 파일 | 어떻게 갱신되나 | 확인 |
| --- | --- | --- | --- |
| 손으로 고정 | `Contracts/upstream.lock.json` (`contract.manifestSha256`) | 유일하게 직접 편집하는 자리 | — |
...
codegen 재생성 → 결정성 확인이 한 묶음이다. 나머지 셋은 전부 파생물이고, 그중
어느 것이든 손으로 편집하면 자기 생성기와 어긋난다. 특히 네 번째를 "우리가 갱신할
것"으로 착각하면 evidence를 lock에 맞춰 편집하게 되는데, 그것은 provenance 게이트가
정확히 잡으려는 행위다([P1](#p1)).
...
| cs-mzv14 (w-0r0ya) | 3 (cold 1) | 4 | 0 / 4 | P14 |
...
finding 4건은 전부 등록부가 아직 닿지 못하던 곳이었고, 그중 3건이 **등록부 자신의
결함**이다. r1은 [P2](#p2)가 틀렸다는 것(세 곳이 아니라 11개 파일)과 라우팅 검사의
앵커 중복 빈틈([P14](#p14))을 잡았다. r2는 그 두 수정이 각각 다시 틀렸다는 것을
잡았다 — P2를 고치며 `MANIFEST.json`을 손으로 갱신하는 파일로 분류했으나 실제로는
`derive-expected-values.py`의 산출물이고, 앵커 중복 검사는 헤딩 계수를 앵커 조건
안에 두어 앵커 없는 헤딩을 아예 못 봤다.

세 라운드가 같은 근본 원인을 가리킨다: **저장소 사실을 확인 없이 단언했다.** 그래서
[항목이 지켜야 할 것](#항목이-지켜야-할-것)이 규칙으로 들어왔다. 라운드 수가 8에서
3으로 줄었지만 3이 아니라 1이 목표다.
